...
public class copperAnvil extends stoneAnvil {

    public copperAnvil(int par1) {
        super(par1);
        setUnlocalizedName("copper_anvil");
        setHardness(3F);
        setPicksEffectiveOn();
    ...
    {
        return RefamishedItems.copperIngot.itemID;
    }
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int iFacing, float fXClick, float fYClick, float fZClick) {
        if (!world.isRemote && !WorldUtils.doesBlockHaveLargeCenterHardpointToFacing(world, i, j + 1, k, 0) && player instanceof EntityPlayerMP) {
            ...
            stoneAnvilContainer container = new stoneAnvilContainer(player.inventory, (copperAnvilTile) world.getBlockTileEntity(i,j,k));
            RefamishedMod.serverOpenCustomInterface((EntityPlayerMP)player, container, RefContains.copperAnvil);
        }
        return true;
    }
    ...
    public void dropBlockAsItemWithChance( World world, int i, int j, int k, int iMetaData, float fChance, int iFortuneModifier )
    {
        ...
        if ( !world.isRemote )
        {
            this.dropItemsIndividually( world, i, j, k, RefamishedItems.copperIngot.itemID, 7, 0, fChance );
        }
    }
    ...
    public void registerIcons(IconRegister register) {
        ...
        this.side = register.registerIcon("refamished:variant/copper_anvil_side");
        this.top = register.registerIcon("refamished:variant/copper_anvil_top");
        this.front = register.registerIcon("refamished:variant/copper_anvil_front");
    }
