...
public class ForgePlanPacketHandler implements CustomPacketHandler {
    ...
    public void handleCustomPacket(Packet250CustomPayload packet, EntityPlayer player) {
        ...
        try {
            ...
            if (packetId == 1) {
                ...
                List<ItemStack> recipeInputs = selected.getDynamicInput(Arrays.asList(tile.inventory));
                ItemStack outputItem = selected.getOutput(Arrays.asList(tile.inventory));

                //if (!baseMatches(base, expectedBase)) return;

                if (!hasAllRequiredInputs(tile, recipeInputs,selected.isCombinationRecipe())) {
                    //System.out.println("AHHHHHHHH");
                    return;
                }
                String nbt = selected.getOutputNbt(Arrays.asList(tile.inventory));
                consumeRequiredInputs(tile, recipeInputs,selected.isCombinationRecipe());

                int metaPlan = getIngotMetaPlan(base);
                if (metaPlan == 16) {
                    metaPlan = getIngotMetaPlan(outputItem);
                }
                ...
                tag.setString("PlanOutput", outputItem.getUnlocalizedName());
                tag.setInteger("PlanAmount", outputItem.stackSize);
                tag.setInteger("ShatterMax", selected.forgeTime);
                tag.setInteger("Mistake", 1);
                tag.setInteger("Metadata", outputItem.getItemDamage());
    ...
    private int getIngotMetaPlan(ItemStack stack) {
        ...
        if (itemThing instanceof SwordItem swordItem) {
            ...
            String un = itemThing.getUnlocalizedName(stack);
            ...
            un.contains("iron") ? 1 :un.contains("tipped") ? 1 : un.contains("gold") ? 2 : un.contains("cobalt") ? 3 : un.contains("gilded") ? 4 : un.contains("diamond") ? 5
...
//                id == copperNugget ? 0 : id == ironNugget ? 1 : id == goldNugget ? 2 : id == gildedNugget ? 4 : 16;
    ...
    private boolean hasAllRequiredInputs(IInventory inv, List<ItemStack> required, boolean isCombination) {
        ...
        for (ItemStack req : required) {
            ...
            for (ItemStack stack : invCopy) {
                ...
                if (stack.itemID != req.itemID) continue;
                if (!isCombination && stack.getItemDamage() != req.getItemDamage()) continue;
    ...
    private void consumeRequiredInputs(IInventory inv, List<ItemStack> required, boolean isCombination) {
        for (ItemStack req : required) {
            ...
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if (s.itemID != req.itemID) continue;
                if (!isCombination && s.getItemDamage() != req.getItemDamage()) continue;
