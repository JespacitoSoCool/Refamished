...
public class rawIngot extends Item {
    public rawIngot(int iItemID) {
        super(iItemID);
        this.setUnlocalizedName("raw_ingot");
        this.setCreativeTab(CreativeTabs.tabMaterials);
        setMaxDamage(0);
        setHasSubtypes(true);
    }
    private static final String[] ingot = new String[]{"copper","iron","gold"};
    ...
    public void registerIcons(IconRegister register) {
        for (int i = 0; i < ingot.length; i++) {
            this.iconByMetadataArray[i] = register.registerIcon("refamished:metallurgy/ingots/raw_" + ingot[i]);
        }
    }
