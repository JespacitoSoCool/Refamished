...
public class horsehideItem extends Item {
    public horsehideItem(int iItemID) {
        super(iItemID);
        this.setUnlocalizedName("horsehide");
    ...
    public void registerIcons(IconRegister register) {
        for (int i = 0; i < hideName.length; i++) {
            this.iconByMetadataArray[i] = register.registerIcon("refamished:variant/horsehide_" + hideName[i]);
        }
    }
