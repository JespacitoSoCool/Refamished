...
public class ribCarvingMachete extends leatherWorkFlint implements IconLargedMultipleRender {
    public ribCarvingMachete(int iItemID) {
        super(iItemID);
    }
    ...
    public Icon getIconFromDamageForRenderPass(int damage, int renderPass) {
        if (renderPass == 0) {
            if (damage == 0) {
                ...
                if (m_recipientIcon[4 - progressIndex] == null) {
                    return m_recipientIcon[0];
                }
    ...
    public int getColorFromItemStack(ItemStack stack, int iRenderPass )
    {
        if ( iRenderPass == 1 )
        {
            return pickaxeDecorated.GetColor(stack);
        }
