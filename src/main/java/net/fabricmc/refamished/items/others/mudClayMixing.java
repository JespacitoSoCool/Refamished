...
public class mudClayMixing extends ProgressiveCraftingItem {
    public mudClayMixing(int iItemID) {
        super(iItemID);
        setUnlocalizedName( "mud_mixing" );
    }
    ...
    public ItemStack onEaten( ItemStack stack, World world, EntityPlayer player )
    {
        ...
        SkillManager.addExperience(player,"Weaving", 1);

        UtilItem.GivePlayerStackOrEject( player, new ItemStack(Item.bucketEmpty));
        return new ItemStack( RefamishedItems.clay_mud, 6 );
    }

    public Icon[] m_recipientIcon = new Icon[7];
    ...
    public void registerIcons( IconRegister register )
    {
        ...
        for (int i = 0; i < m_recipientIcon.length; i++) {
            m_recipientIcon[i] = register.registerIcon("refamished:progressive/clay_mixing_bucket_" + i);
        }
    ...
    public Icon getIconFromDamage(int damage) {
        if (damage == 0) {
            ...
            return m_recipientIcon[6 - progressIndex];
        }
    }
