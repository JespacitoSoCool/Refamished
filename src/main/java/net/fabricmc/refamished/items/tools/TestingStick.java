...
public class TestingStick extends Item {
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int i, int j, int k, int iFacing, float fClickX, float fClickY, float fClickZ )
    {
        ...
        else if (the_block instanceof steamContainer block) {
            TileEntity tile = world.getBlockTileEntity(i,j,k);
            if (tile instanceof copperConductTile conduct && !world.isRemote) {
                player.addChatMessage("Steam : "+conduct.getSteamAmount());
                //System.out.println("Steam : "+conduct.getSteamAmount());
            }
        }
        else
        {
    ...
    public Multimap getItemAttributeModifiers() {
        ...
        var1.put(ResharedMonsterAttributes.toolRange.getAttributeUnlocalizedName(), new AttributeModifier(RefamishedAddon.toolRangeUUID, "Tool Range", 15, AttributeOperation.ADDITIVE.value));
        var1.put(ResharedMonsterAttributes.attackRange.getAttributeUnlocalizedName(), new AttributeModifier(RefamishedAddon.toolRangeUUID, "Attack Range", 15, AttributeOperation.ADDITIVE.value));
        return var1;
    }
}